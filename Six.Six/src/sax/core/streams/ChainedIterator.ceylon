...
"""
An [[Iterator]] that returns the elements of two [[Iterable]]s, as if they were
chained together.
"""
see (function Iterable.chain)
tagged("Streams")
class ChainedIterator<out Element, out Other>({Element*} first, {Other*} second) is Iterator<Element|Other>
{
    ...
    value string => "``first``.chain(``second``).iterator()";
}
