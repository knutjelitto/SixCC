...
shared interface Map<out Key=Object, out Item=Anything>
    ...
    "A bag of items."
    class Items() extends Object() satisfies Collection<Item>
    {
    ...
    "Two maps are considered equal iff they have the same _entry sets_. The entry set of a `Map` is
    ...
    "Produces a map with the same [[keys]] as this map. For every key, the item is the result of
            ...
            satisfies Map<Key,Result>
    {
        ...
        getOrDefault<Default>(Object key, Default default)
        {
            if (is Key key)
            {
                return
    ...
    "Produces a map containing the elements of this map, after replacing every `null` item in the
    ...
    defaultNullItems<Default>(
        ...
        => mapItems( (key, elem) => elem else defaultValue );
    
    "Produces a map by applying a [[filtering]] function to the [[keys]] of this map. This is a
     lazy operation, returning a view of this map."
    since("1.1.0")
    shared default 
    Map<Key,Item> filterKeys(
        "The predicate function that filters the keys of this map, determining if there is a
    ...
    "Produces a map whose keys are the union of the keys of this map, with the keys of the given
    ...
    patch<OtherKey,OtherItem>(Map<OtherKey,OtherItem> other) 
        given OtherKey satisfies Object
        => object 
        extends Object()
        satisfies Map<Key|OtherKey,Item|OtherItem>
    {
        defines(Object key) => other.defines(key) || outer.defines(key);
        ...
        getOrDefault<Default>
            (Object key, Default default) 
            => switch (result = other.lookup(key))
            case (Missing.instance) outer.getOrDefault(key, default)
            else result;
        ...
        contains(Object entry)
            => if (is Object->Anything entry)
            then entry in other || !other.defines(entry.key) && entry in outer
            else false;
        
        //efficient when map is much smaller than outer,
        //which is probably the common case 
        size => outer.size + other.keys.count(not(outer.defines));
    ...
    Map<Key,Item&Object> coalescedMap
        => object extends Object() satisfies Map<Key,Item&Object>
        {
...
shared Map<Key,Item> map<Key,Item>(
    ...
    {<Key->Item>*} stream,
    "A function that chooses between items with duplicate keys. By default, the item that occurs
...
tagged("Collections")
shared object emptyMap extends Object()
...
{    
    ...
    shared actual Map<Nothing, Nothing> mapItems<Result>(Result mapping(Nothing key, Nothing item)) => emptyMap;
    ...
    shared actual Null find(Boolean selecting(Nothing->Nothing element)) => null;
    
    shared actual Null findLast(Boolean selecting(Nothing->Nothing element)) => null;
