...
"""
Given two streams, form a new stream consisting of all [[entries|Entry]] where, for any given index in the resulting
stream, the [[key|Entry.key]] of the entry is the element occurring at the same index in the [[first stream|keys]], and
the [[item|Entry.item]] is the element occurring at the same index in the [[second stream|items]]. The length of the
resulting stream is the length of the shorter of the two given streams.
...
first element of the pair is the element occurring at the same index in the [[first stream|firstElements]], and the
second element of the pair is the element occurring at the same index in the [[second stream|secondElements]]. The
length of the resulting stream is the length of the shorter of the two given streams.
...
values in the [[first stream|heads]] to the tuples in the [[second stream|tails]]. The length of the resulting stream
...
zip<Element,Head,Tail,HeadAbsent,TailAbsent>(Iterable<Head,HeadAbsent> heads, Iterable<Tail,TailAbsent> tails)
    given Tail satisfies Element[]
    given HeadAbsent satisfies Null
    given TailAbsent satisfies Null
    => mapPairs(heads, tails, Tuple<Element|Head,Head,Tail>);
