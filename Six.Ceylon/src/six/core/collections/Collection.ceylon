...
tagged("Collections")
shared interface Collection<out Element=Anything> satisfies {Element*}
{    
    """
    ...
    shared {[Element+]*} permutations 
        => object satisfies {[Element+]*}
        {
            value multiset =
                ...
                .indexed
                .flatMap((index->entries) => entries.map((_->item) => index->item));
            ...
            iterator() => object satisfies Iterator<[Element+]>
            {
                ...
                shared actual [Element+]|Finished next()
                {
                    ...
                    else if (exists i -> [key->_, __] = elements.paired.locateLast(([m->_, n->__]) => m < n))
                    {
                        assert (exists j = elements.lastIndexWhere((k->_) => k > key));
                        elements.swap(i,j);
    ...
    - one element is an `Object` and the other is `null`.
    """
    throws (class AssertionError, "if [[length]] is nonpositive or if `length` is larger than the number of distinct elements of this collection")
