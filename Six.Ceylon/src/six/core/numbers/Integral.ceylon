...
The division operation for integral numeric types results in a remainder. Therefore, integral numeric types have 
[[an operation|remainder]], denoted by the _remainder_ operator `%`, to determine the remainder of any division
operation.
...
All `Integral` numeric types are also [[Enumerable]], so ranges of integral values may be produced using the
...
"""
see (class Integer) by ("Gavin") tagged("Numbers")
shared interface Integral<Other> of Other
    satisfies Number<Other> & Enumerable<Other>
