...
"""
Represents a path in a hierarchical file system. A path is a sequence of path elements. A path may
be an absolute path that begins at the root of the file system, or a relative path.
"""
shared sealed interface Path satisfies Comparable<Path>
{    
    "The parent path of this path, formed by removing the last path element, or, if this path is
     the root path, and has no path elements, this root path."
    ...
    "This path, converted into an absolute path. If  this path is already absolute, return this
     path. Otherwise, if this path is a relative path, resolve it against the file system's default
     directory."
    ...
    "Obtain a `Resource` representing the file or directory located at this path.
     ...
     If a symbolic link exists at this path, and the symbolic link ultimately points to a file or
     directory, a `File` or `Directory` resource will be returned rather than a `Link` resource."
    shared formal Resource resource;
    
    "Obtain a `Link` representing the symbolic link located at this path, or `null` if a symbolic
