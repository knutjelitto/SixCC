...
shared sealed interface ReadableFileDescriptor satisfies Closeable
{
    "Reads everything we can from this file descriptor into the specified buffer.
     ...
     If this file descriptor is in `blocking` mode, it will block the current thread until the
     buffer is full, or until we reached end of file.
     
     If this file descriptor is in `non-blocking` mode, it will only read the data that is
     available for reading without blocking, which may be less than the buffer's available space.
    ...
    "Reads data until the end of file is reached, in a blocking way, by passing it to the specified
     consumer. This method makes no sense if the file descriptor is in `non-blocking` mode."
...
shared sealed interface WritableFileDescriptor satisfies Closeable
{
    "Writes everything we can from the specified buffer to this file descriptor.
     ...
     If this file descriptor is in `blocking` mode, it will block the current thread until the
     buffer is written entirely, or until we reached end of file.
     
     If this file descriptor is in `non-blocking` mode, it will only write the data that can be
     written without blocking, which may be less than the buffer's available data.
    ...
    "Writes the given buffer to this file descriptor entirely, until either the buffer has been
     entirely written, or until end of file. This method makes no sense if the file descriptor is
    ...
    "Writes all the data produced by the given producer to this file descriptor, until the producer
     stops filling the buffer, or end of file. This method will repeatedly invoke the producer so
     that it can push data to the buffer. If the producer wants to indicate end of input, it only
     has to stop adding data to the buffer. This method makes no sense in `non-blocking` mode."
...
see (`interface Socket`, `interface SelectableFileDescriptor`)
by ("Stéphane Épardaud")
shared sealed interface FileDescriptor satisfies ReadableFileDescriptor & WritableFileDescriptor
{
}
