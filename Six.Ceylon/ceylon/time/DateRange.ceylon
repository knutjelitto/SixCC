...
{
    "The first Element returned by the iterator, if any. This should always produce the same value
     as `iterable.iterator().head`. It also represents the _caller_ that created the Range:
    ...
    "Customized way to iterate over each element, it does not interfer in _from_ and _to_ fields,
     but it does not guarantee that _to_ will be included in iterator."
    ...
    shared actual Duration duration => Duration((to.dayOfEra - from.dayOfEra) * milliseconds.perDay);
    ...
    "An iterator for the elements belonging to this container. where each jump is based on actual
     step of this Range."
    ...
    "The iteration for each element should always start from same point, this way is possible to
     not suffer with different number of days in months."
