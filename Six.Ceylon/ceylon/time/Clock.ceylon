...
Applications should _avoid_ using the top level objects directly. Instead, they should pass a
[[Clock]] into any method that requires it. A dependency injection framework is one way to achieve
this.

This approach allows an alternate clock, such as [[fixedTime]] to be used during testing.

The [[systemTime]] top level factory method offers clocks based on the best available system clock,
such as `system.milliseconds`.
"""
...
"Returns an implementation of a clock that always returns a constant offset from the value of the
 provided clock."
shared Clock offsetTime(Clock baseClock, Duration offset)  => OffsetClock(baseClock, offset);

"An implementation of a [[Clock]] that returns time with a constant offset from the provided clock."
