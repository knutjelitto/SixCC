...
"API for accessing hierarchical file systems. Clients use [[Path]]s to obtain [[Resource]]s
 ...
 Note the difference between a [[File.Overwriter]], which destroys the existing
 contents of the file, if any, and a [[File.Appender]], which leaves them intact.
 ...
 To list the contents of a directory, we have two possibilities. We can list just
 the direct contents:
 ...
 Alternatively, we can create a [[Visitor]] that walks the whole directory tree
 ...
 File systems other than the default file system are supported. For example, a file
 system for a zip file may be created using the convenience function
 [[createZipFileSystem]].
