...
static void rrd_print_dot(const char *prefix, const void *parent, const char *port,
	const struct node *node)
{
	...
	case NODE_CI_LITERAL:
		...
		escputt(&node->u.literal, stdout, escputc);
		printf("\\\"\"/i");
		break;
	...
	case NODE_RULE:
		printf("label = \"\\<");
		escputs(node->u.name, stdout, escputc);
		printf("\\>\"");
		break;
	...
	case NODE_PROSE:
		...
		printf("label = \"?");
		escputs(node->u.prose, stdout, escputc);
		printf("?\"");
		break;
