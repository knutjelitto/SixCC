...
void svg_path_h(struct path **paths, unsigned x, unsigned y, int n)
{
    struct path *nuw;
...
void svg_path_v(struct path **paths, unsigned x, unsigned y, int n)
{
    struct path *nuw;
...
void svg_path_q(struct path **paths, unsigned x, unsigned y, int rx, int ry, int mx, int my)
{
    struct path *nuw;
...
void svg_path_move(struct path *n, unsigned *x, unsigned *y)
{
    assert(n != NULL);
    assert(x != NULL);
    assert(y != NULL);
...
struct path* svg_path_find_preceding(struct path* paths, const struct path* n)
{
    struct path* p;
...
struct path * svg_path_find_start(struct path *paths)
{
    struct path *p;
...
static void svg_path_free_one(struct path *n)
{
    assert(n != NULL);
...
void svg_path_remove(struct path **paths, struct path *n)
{
    struct path **p;
...
static void svg_path_merge(struct path **paths, struct path *p, struct path *q)
{
    assert(paths != NULL);
    assert(p != NULL);
    assert(q != NULL);
...
void svg_path_consolidate(struct path **paths)
{
    struct path *p, *q;
