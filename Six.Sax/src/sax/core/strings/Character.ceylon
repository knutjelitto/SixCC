...
Every `Character` has a unique [[Integer]]-valued Unicode _code point_.
...
Characters have a [[natural order|Comparable]] determined by their Unicode code points. So, for example, `'a' < 'b'`,
...
class Character(Character character)
    extends Object()
    satisfies Comparable<Character> & Enumerable<Character>
{
    """
    ...
    Conversion of uppercase characters to lowercase is performed according to a locale-independent mapping that produces
    incorrect results in certain locales (e.g. `tr-TR`).
    
    Furthermore, this conversion always produces a single character, which is incorrect for characters whose uppercase
    representation comprises multiple characters, for example \{LATIN CAPITAL LETTER I WITH DOT ABOVE}. Thus,
    
    - `'\{LATIN CAPITAL LETTER I WITH DOT ABOVE}'.lowercased` evaluates to `'i'`, whereas
    - `"\{LATIN CAPITAL LETTER I WITH DOT ABOVE}".lowercased` evaluates, more correctly, to the string  `"i\{COMBINING DOT ABOVE}"`.
    
    Therefore, for most purposes, it is better to use `char.string.lowercased` instead of `char.lowercased`.
    """
    see (value String.lowercased, value Character.uppercased)
    shared native Character lowercased;
    
    """
    The uppercase representation of this character.
    
    Conversion of lowercase characters to uppercase is performed according to a locale-independent mapping that produces
    incorrect results in certain locales (e.g. `tr-TR`).
    
    Furthermore, this conversion always produces a single character, which is incorrect for characters whose uppercase
    representation comprises multiple characters, for example \{LATIN SMALL LETTER SHARP S}. Thus,
    
    - `'\{LATIN SMALL LETTER SHARP S}'.uppercased` evaluates to `'\{LATIN SMALL LETTER SHARP S}'`, whereas
    - `"\{LATIN SMALL LETTER SHARP S}".uppercased` evaluates, more correctly, to the string `"SS"`.
    
    Therefore, for most purposes, it is better to use `char.string.uppercased` instead of `char.uppercased`.
    """
    see (value String.uppercased, value Character.lowercased)
    ...
    "The hash code for this `Character`, which is always its 32-bit [[Unicode code point|integer]]."
    ...
    """
    Determine if this character's code point is larger than the given character's code point, returning `false` if the two
    characters represent the same code point.
    """
    shared actual native Boolean largerThan(Character other)
        => this.integer > other.integer;
     
    """
    Determine if this character's code point is smaller than the given character's code point, returning `false` if the two
    characters represent the same code point.
    """
    ...
    """
    Determine if this character's code point is larger than or equal to the given character's code point, returning `true`
    ...
    """
    Determine if this character's code point is smaller than or equal to the given character's code point, returning `true`
    if the two characters represent the same code point.
    """
