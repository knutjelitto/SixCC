...
"""
Abstraction of types which support a multiplication operator `x * y`. For [[numeric types|Numeric]], this is just
familiar numeric multiplication.

    Float c = 2 * pi * r;

A concrete class that implements this interface should be a mathematical _semigroup_. That is, the multiplication
operation should be associative, satisfying:

- `(x*y)*z == x*(y*z)`

A `Multiplicable` type might be a _monoid_, that is, a semigroup with an additive identity element, usually denoted `1`,
...
- `x * 1 == x`
"""
see (interface Numeric) by ("Gavin") tagged("Numbers")
