...
"""
An 8-bit byte. A `Byte` value represents a congruence class of [[integers|Integer]] modulo 256, and may be interpreted
as:
...
`Byte`s with modular addition form a [[mathematical group|Invertible]]. Thus, every byte `b` has an additive inverse
`-b` where:

    (-b).signed == -b.signed
    (-b).unsigned == b.unsigned==0 then 0 else 256 - b.unsigned

`Byte` is a [[recursive enumerable type|Enumerable]]. For example, the range:

254.byte .. 1.byte

contains the values `254.byte, 255.byte, 0.byte, 1.byte`.

`Byte` does not have a [[total order|Comparable]] because any such order would:
...
Thus, to compare the magnitude of two bytes, it is necessary to first convert them to either their `signed` or
...
shared native final class Byte(congruent)
    extends Object()
    satisfies Binary<Byte> & Invertible<Byte> & Enumerable<Byte>
{
    """
    ...
    shared native Integer unsigned = (congruent % #100).and(#FF);
    ...
    """
    shared actual native Byte plus(Byte other) => (this.unsigned + other.unsigned).byte;
    
    function indexInRange(Integer index) => 0 <= index < 8;
    
    function mask(Integer index)  => 1.byte.leftLogicalShift(index);
    ...
    shared actual native Byte set(Integer index, Boolean bit)
        =>  indexInRange(index)
            then (bit then or(mask(index)) else and(mask(index).not))
            else this;
    ...
    shared actual native Byte and(Byte other) => unsigned.and(other.unsigned).byte;
    
    shared actual native Byte or(Byte other) => unsigned.or(other.unsigned).byte;
    
    shared actual native Byte xor(Byte other) => unsigned.xor(other.unsigned).byte;
    
    """
    If [[shift]] is in the range `0..$111`, shift the bits to the left by `shift` positions, using zero extension to fill
    in the least significant bits. Otherwise shift the addressable bits to the left by `shift.and($111)` positions, using
    zero extension.
    """
    aliased ("leftShift")
    ...
    """
    If [[shift]] is in the range `0..$111`, shift the bits to the right by `shift` positions, using zero extension to fill
    in the most significant bits. Otherwise shift the addressable bits to the right by `shift.and($111)` positions, using
    zero extension.
    """
    aliased ("rightShift")
    ...
    """
    If [[shift]] is in the range `0..$111`, shift the bits to the right by `shift` positions, using sign extension to fill
    in the most significant bits. Otherwise shift the addressable bits to the right by `shift.and($111)` positions, using
    sign extension.
    """
