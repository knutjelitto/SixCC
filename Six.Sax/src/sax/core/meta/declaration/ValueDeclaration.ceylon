...
Getting a model requires applying type arguments to the value declaration with [[apply]] in order
to be able to read that value. For example, here is how you would obtain a value model that you can
read from a toplevel attribute declaration:
...
For attributes it is a bit longer, because attributes need to be applied the containing type, so
you should use [[memberApply]] and start by giving the containing closed type:
...
"""
shared sealed interface ValueDeclaration satisfies FunctionOrValueDeclaration & GettableDeclaration
{
    ...
    "Applies this value declaration in order to obtain a value model. See
     [this code sample](#toplevel-sample) for an example on how to use this."
    ...
    "Applies the given closed container type to this attribute declaration in order to obtain an
     attribute model. See [this code sample](#member-sample) for an example on how to use this."
    ...
    shared actual default Anything get() => apply<Anything, Nothing>().get();
