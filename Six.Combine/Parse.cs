...
namespace Six.Combine
{
    ...
    public static partial class Parse
    {
            ...
            return input =>
            {
                if (!input.AtEnd)
                {
                    if (predicate(input.Current))
                        return Result.Success(input.Current, input.Advance());

                    return Result.Failure<char>(input,
                        $"unexpected '{input.Current}'",
                        new[] { description });
                }
        ...
        public static Parser<object?> Not<T>(this Parser<T> parser)
        {
            return input =>
            {
                ...
                if (result.WasSuccessful)
                {
                    ...
                    return Result.Failure<object?>(input, msg, Array.Empty<string>());
                }
        ...
        public static Parser<U> Then<T, U>(this Parser<T> first, Func<T, Parser<U>> second)
        {
            return input =>
            {
                return first(input).IfSuccess(s => second(s.Value)(s.Remainder));
            };
        }
        ...
        public static Parser<IEnumerable<T>> Many<T>(this Parser<T> parser)
        {
            return input =>
            {
                ...
                while (parse.WasSuccessful)
                {
                    if (remainder.Equals(parse.Remainder))
                        break;

                    items.Add(parse.Value);
                    remainder = parse.Remainder;
                    parse = parser(remainder);
                }

                return Result.Success<IEnumerable<T>>(items, remainder);
            };
        }
        ...
        /// <param name="parser"></param>
        /// <returns></returns>
        public static Parser<IEnumerable<T>> XOnOrMore<T>(this Parser<T> parser)
        {
        ...
        {
            return input => parser(input).IfSuccess(s =>
                s.Remainder.AtEnd
                    ? s
                    : Result.Failure<T>(
                        s.Remainder,
            ...
            return input =>
                       {
                           ...
                           {
                               var pResult = (input.Memos[p] as IResult<T>)!;
                               if (pResult.WasSuccessful)
                                   return pResult;
                               throw new ParseException(pResult.ToString()!);
                           }

                           input.Memos[p] = Result.Failure<T>(input,
        ...
        public static Parser<T> Or<T>(this Parser<T> first, Parser<T> second)
        {
            ...
            return input =>
            {
                ...
                if (!fr.WasSuccessful)
                {
                    return second(input).IfFailure(sf => DetermineBestError(fr, sf));
                }

                if (fr.Remainder.Equals(input))
                    return second(input).IfFailure(sf => fr);
        ...
        {
            return input => parser(input).IfFailure(f => f.Remainder.Equals(input) ?
        ...
        public static Parser<T> XOr<T>(this Parser<T> first, Parser<T> second)
        {
            return input => {
                ...
                if (!firstParse.WasSuccessful)
                {
                    // The 'X' part
                    if (!firstParse.Remainder.Equals(input))
                        return firstParse;

                    return second(input).IfFailure(secondParse => DetermineBestError(firstParse, secondParse));
                }

                // This handles a zero-length successful application of first.
                if (firstParse.Remainder.Equals(input))
                {
                    return second(input).IfFailure(secondParse => firstParse);
                }
        ...
        {
            return input => Result.Success(value, input);
        }
        ...
        {
            return input => parser(input).IfSuccess(s =>
                predicate(s.Value) ? s : Result.Failure<T>(
                    input,
                    $"Unexpected {s.Value}.",
                    Array.Empty<string>()));
        }
        ...
        /// Parse a number.
        /// </summary>
        public static readonly Parser<string> Number = Numeric.OneOrMore().Text();
